...
from config.constants import PRESENTATION_THEMES, EXPORT_FORMATS
from utils.exporters import export_to_pdf, export_to_pptx
...
def render_sidebar():
    """Render sidebar with presentation list and actions"""
...
def render_presentation_editor(db):
    """Render the main presentation editor"""
        ...
        if st.button("📄 Export to PPTX"):
            if st.session_state.slides:
                try:
                    ...
                    st.success("PPTX exported successfully!")
                except Exception as e:
